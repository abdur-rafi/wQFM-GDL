...
if [[ -z "$MODE" ]]; then
    echo "Error: Mode not specified. Use -t (tree-based) or -q (quartet-based)."
    usage
    exit 1
fi
...
if [[ "$MODE" == "t" ]]; then
    ...
    if [[ ! -f "$QUARTET_GEN_JAR" ]]; then
        echo "Error: JAR file not found at '$QUARTET_GEN_JAR'."
        echo "Please build it first:"
        echo "  javac -d bin \$(find src -name '*.java')"
        echo "  echo 'Main-Class: src.QuartetGenMain' > manifest.txt"
        echo "  jar cfm QuartetGenMain.jar manifest.txt -C bin ."
        exit 1
    fi
    if [[ ! -f "$WQFM_Q_JAR" ]]; then
        echo "Error: JAR file not found at '$WQFM_Q_JAR'."
        echo "Expected structure: wQFM-v1.4/wQFM-v1.4.jar and wQFM-v1.4/lib/"
        exit 1
    fi
fi
...
# Pipeline
# ---------------------------------------------------------------------------
MODE_LABEL="$([ "$MODE" = "t" ] && echo "wQFM-GDL-T (tree-based)" || echo "wQFM-GDL-Q (quartet-based)")"
...
# Step 2 — Resolve polytomies  [common to both modes]
echo "[2] Resolving polytomies..."
...
# Step 3 — Clean the resolved output  [common to both modes]
echo "[3] Cleaning resolved trees..."
...
if [[ "$MODE" == "q" ]]; then
    ...
    # -----------------------------------------------------------------------
    # wQFM-GDL-T: tree-based pipeline
    # -----------------------------------------------------------------------
